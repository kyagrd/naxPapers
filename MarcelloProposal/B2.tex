...
\]
One %and one
intuitively explains the correspondence along 
...
project %{\erc} 
builds %is to build 
a group dedicated to combined research in Category Theory,
...
To make substantial progress in the area, we will systematically explore a
broad spectrum of key features present in type theories.  The scale at which
we will be attempting this is graphically represented by the research space of
investigation in Figure\,\ref{TypeTheoryResearchSpace}, 
\begin{figure}[h]
...
Fiore~\etal~\cite{FiorePlotkinTuri,FioreLICS08,FioreHurLMCS,FioreSecOrdEqLog,FioreMahmoud,FioreICALP},
...
\item
  To synthesise metalanguages for type theories in the form of formal systems
  for equational deduction, that are sound and complete for the model theory.
...
investigate a main problem in the area: 
\begin{resenumerate}\setcounter{CC}{0}
\item
...
henceforth referred to as judgemental equality.
...
\item
  ...
  extension results for Martin-L\"of Type Theory (and, more generally, Pure
  Type Systems).  
\end{resenumerate}
