...
\item[\raisebox{.75mm}{\tiny$\bigstar$}]\hspace*{-2mm}
  To identify %search for %elucidate 
  %what we hope to be 
...
led to inconsistency.  He was led to this %contradiction 
...
\end{array}
\]
of Propositional Intuitionistic %Minimal 
Logic. %~\cite{?}.
...
Combinatory Logic\footnote{Combinatory Logic is an important symbolic
  ...
  deduction system for Propositional Intuitionistic %Minimal 
  Logic.} 
...
We %Having introduced the scientific framework, we 
...
plan of action. %research programme.
...
evolution of type theory has been that of Dependent Type Theory.
Dependent judgements and dependent types were introduced by
de~Bruijn~\cite{deBruijn} in the 1960s as an extension of their simple
versions allowing types to be indexed (or parameterised) by other types.
He was motivated by the formalisation of mathematical theories, where such
objects abound.  For instance, in combinatorics one is interested in the
type of permutations $\mathfrak S(n)$ on $n$ elements, as the index (or
parameter) $n$ ranges over the natural numbers $\mathbb N$.  In modern
notation, this is presented by a judgement of the form
\[
...
This is the subject of Section\pref{CalculiSubsection}.
...
found in programming languages as indexed datatypes, like the type
$\mbox{\small$\mathsf{Vec}$}\:\alpha\:n$ of \mbox{$n$-dimensional} vectors
of type $\alpha$.  
%
Programming with indexed datatypes will be generically referred to as
indexed programming.  

Indexed programming is being developed from one of two main influences
(none to do with presheaves though): 
%
the practical needs of supporting data structures able to maintain strong
data invariants, like nested %~\cite{?} 
and generalised algebraic datatypes (GADTs) %~\cite{?}
in functional programming~\cite{Omega,Haskell}; and 
%
the experimentation with dependently-typed programming
languages~\cite{Cayenne,Epigram} as a by-product of dependent type theory.
%
Since %the intrinsic concerns of each of 
these two views somehow pull the design of indexed programming languages
in contrasting directions, one is lead to reconsider the approach as
follows.
...
Plotkin~\cite{BarberPlotkin}.  In the context of effect calculi, examples
are the Call-By-Push-Value of Levy~\cite{LevyCBPV} and the Enriched Effect
Calculus of Egger, M{\o}gelberg, and Simpson~\cite{EEC}.  
%
Both these lines of work regard resources and effects as dual categorical
structures, resource comonads and computational monads, arising at
opposite sides of an adjunction, intuitively between linear and cartesian
structures or between computations and values.  
%
However, this is not so in all models; and the following question remains
unanswered.
%
...
research programme. %plan of action.
...
\item\label{ATTGenericProgramming}
  To test the above mathematical theories by formalising them in 
  Coq or Agda %a proof assistant, say Coq or Agda, 
  and to apply them for the purpose of generic programming for type theories
  (see \eg~\cite{DybjerGP}), while at the same time exercising these systems
  to the limit to identify shortcomings that will trigger new research in the
  context of proof assistants.
\end{resenumerate}
...
(as detailed in
item~\ref{MethodologyMathematicalUniversesParagraph}\thinspace\itemref{ConstructionsOnTypeTheories}
...
item~\mbox{\ref{MethodologyMathematicalUniversesParagraph}\thinspace\itemref{MMUPAppItem}}
above.
...
item~\ref{PolarisationParagraph}\thinspace\itemref{ItemFour} above.
...
\end{resenumerate}
We are specially interested in grounding it through categorical models (see
Figure\,\ref{ResearchAreas}), that have been scarcely investigated in this
context.
...
%  Combinatory Logic 
%
Mendler's Mul\-ti\-mod\-al CK~\cite{MendlerMMCK};
and
%
Park and Im's Calculus $S_\Delta$~\cite{ParkIm}.
%
A particularly interesting aspect of many of these systems is that they have
reflective features~\cite{ReflectionSmith}, in that they can internally
represent and manipulate themselves.  In this respect, and in connection to
the proposed research in Section\pref{Programming}, we will:
\begin{resenumerate}\setcounter{CC}{2}
\item\label{ReflectiveProgrammingLanguages}
  Investigate the role of modal calculi supporting reflection in the design of
...
\item
  Extend
  item~\ref{GeneralisedTypeTheoryParagraph}\thinspace\itemref{GeneralisedTypeTheoryItem}
...
item~\ref{WiringStructureParagraph}\thinspace\itemref{BellantoniCook}).
...
\hide{%
Finally, let us also mention an intriguing possibility 
\hide{%
suggested by the model-theoretic considerations of
...
in the direction of metaprogramming:
\begin{resenumerate}\setcounter{CC}{6}%{7}
\item
  Extend {\SystemFi} with typed self representation based on Indexed
  Abstract Syntax, rather than Higher-Order Abstract Syntax %~\cite{}
  as has been the norm (see \eg~\cite{Rendel}).
\end{resenumerate}
}
...
Metaprogramming is the activity of writing programs that manipulate other
programs.  To support this programming style, a meta\-pro\-gram\-ming system
embodies two languages: the object language, which is the object of study, and
the metalanguage, which is used to 
compute with %define algorithms for the purpose of constructing or analysing
object programs as data.  
%
In some metaprogramming systems, like LISP, %~\cite{}, 
the object language and the metalanguage coincide.  Such languages are
reflective, in that they can manipulate themselves.  Other systems, chiefly
typed ones, support this ability via self representation.

We are interested here in both the activity of meta\-programming and the
development of metaprogramming languages.  

Different kinds of metaprogramming are possible.  In particular, generic
programming 
%One style of generic programming defines a data structure that reflects how
%users define new types. For example, a value of type \verb+Def Tree+ describes
%how a new \verb+Tree+ datatype is defined. A generic program then analyzes
%this structure of \verb+Def+ types to write code parametric over any
%user-defined type. For example a generic equality might have type
%\verb+Def t -> t -> t -> Bool+ and a generic marshalling function might have
%type \verb+Def t -> t -> List Bit+. 
focusses on code that is parametric over user-defined types.  In this context,
we will:
\begin{resenumerate}\setcounter{CC}{0}
\item
  Explore the metamathematical foundations for type theories of the Algebraic
  Type Theory programme (Section~\pref{AlgebraicTypeTheoryParagraph}) to
  develop generic programming for type theories.
\end{resenumerate}
One challenge here is to develop generic substitution programs for binding,
polymorphic, and/or dependent type theories.

As for metaprogramming languages, one possible direction is to:
\begin{resenumerate}\setcounter{CC}{1}
\item
  Extend {\SystemFi} with typed self representation (based on Indexed Abstract
  Syntax, rather than Higher-Order Abstract Syntax %~\cite{}
  as has been the norm).
\end{resenumerate}
More generally, we propose to:
\begin{resenumerate}\setcounter{CC}{2}
\item
  Investigate the role of logical calculi supporting reflection
  (\eg~\cite{AltArtemov}) in programming language design, in particular in the
...
\hidenote{Makoto --- Marco --- Martin --- Michael}
...
%notion of 
model of dependent types with a close connection to the syntax
(that features in Section\mbox{\pref{IntensionalTypeTheoryParagraph}}); 
%
...
definition~\cite{DybjerIR}, that %It extends the notion of inductive
%definition and 
is useful for understanding universes and reflection principles in type theory 
...
(and %which 
is relevant to
Sections\,(\ref{AlgebraicTypeTheoryParagraph}\thinspace\itemref{ATTGenericProgramming}\,\& \mbox{\ref{IndexedProgrammingParagraph}}%\,\&\,%
...
Section\,(\ref{ModalLogicsParagraph}\thinspace\itemref{ReflectiveProgrammingLanguages}\,\&\,\ref{Programming}).
...
\hspace*{30mm}
\vspace*{15cm}
%\includegraphics[width=11cm]{Costs.pdf}
\end{figure*}
...
\bibitem{DybjerGP}
M.\,Benke, P.\,Dybjer, and P.\,Jansson (2003).
\newblock Universes for generic programs and proofs in dependent type theory.
\newblock \emph{Nordic Journal of Computing}, 10:265--269.
