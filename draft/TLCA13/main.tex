...
\begin{figure}
\paragraph{Syntax:}
\qquad~\quad Term Variables\qquad\qquad $x,i$ ~\vskip-4ex
\begin{align*}
&\text{Type Constructor Variables} & X \\
&\text{Sort}
        & \square
        \\
&\text{Kinds}
...
$x$ and $i$ is only conventional, that is, for the sake of readability.
...
to emphasize that $s$ is an index term, which is erasable. Index applications
...
and the type instantiation ($\forall I$) rules, but involves
...
The index instantiation rule ($\forall E i$), we require that
the index term $s$, which to be instantiated for $i$, to be well-typed
...
an index variable ($i$) already in scope (\eg, $\l i^A.F\{s\}$,
$\forall i^A.F\{s\}$).
...
by induction over the judgment\footnote{The proof for
	...
	mutually recursive.
	So, we prove a combined proposition on the judgment $J$,
	which is either a kinding judgment or a typing judgment
	(\ie, $J ::= \Delta |- F : \kappa \mid \Delta;\Gamma |- t : A$),
	such that
	Proposition \ref{prop:wfkind} when $J$ is a kinding judgment, otherwise,
	Proposition \ref{prop:wfkind} holds when $J$ is a typing judgment.}
