 ...
 ~~~~
   \inference{}
      { \MIt^\psi x.\varphi\;(\In^\kappa t) \rightsquigarrow
        \case^\psi~t~\, [\MIt^\psi x.\varphi/x]\varphi }
\]
...
\[ \inference{s\rightsquigarrow s'}
 ...
 ~~~~
 \inference{s\rightsquigarrow s'}
             {\MIt^\psi x.\varphi~s \rightsquigarrow \MIt^\psi x.\varphi~s'}
\]
...
\]
\[ \inference[(MIt)]
      { \Sigma;\Delta;\Gamma |- s : \mu^{\kappa}\,T\;\overline\tau
      & \Sigma;\Delta,X^\kappa;
           ...
           : \forall\,\overline\iota . T\,X\,\overline\iota
                                     -> \psi(\overline\iota) }
      {\Sigma;\Delta;\Gamma
           |- \MIt^\psi x.\varphi ~ s : \psi(\overline\tau) }
      \big( X\notin\FV(\Gamma) \big)
\]
...
$F:\kappa$ is the type constructor binding.
...
Mendler style iterators ($\MIt^\psi\dots$), and
