...
give well-formed results. More specifically, we want to make sure that
three propositions hold: sotring derivations result in well-formed sorts
(Proposition \ref{prop:wfsort}), kinding derivations under well-formed
type level contexts result in well-sorted kinds (Proposition \ref{prop:wfkind}),
and typing derivations under well-formed type and term level contexts result in
...
the proof for the three propositions as if they were separate proofs mutually
referencing each other, since they are easier to understand that way.
But, technically, when I say ``by induction'' during the proofs, what I
really mean is the induction hypothesis of that big conjuncted property.
...
\item[case] ($\forall E$)
	...
	Then, we use the type substitution lemma (\ref{lem:tysubst}).
\item[case] ($\forall Ii$)
	By induction and well-formedness of $\Delta$.
\item[case] ($\forall Ei$)
	...
	that $\Delta,i^A |- B : *$.
	Then, we use the index substitution lemma (\ref{lem:ixsubst}).
\end{itemize}
\end{proof}
