...
\]
\[
  \inference[($\lambda$)]{|- \kappa & \Delta,X^\kappa |- F : \kappa'}
...
\[ \inference[($->$)]{\Delta |- A : * & \Delta |- B : *}
                     {\Delta |- A -> B : * }
 ~~~~
   \inference[($\forall$)]{|- \kappa & \Delta, X^\kappa |- B : *}
...
\[
 ...
 ~~~~ ~~~~
   \inference[($->$$E$)]{\Delta;\Gamma |- r : A -> B & \Delta;\Gamma |- s : A}
                        {\Delta;\Gamma |- r\;s : B}
\]
\[ \inference[($\forall I$)]{|- \kappa & \Delta, X^\kappa;\Gamma |- t : B}
...
\]
\[ \inference{|- \kappa & \Delta,X^\kappa |- F=F' : \kappa'}
...
\[ \newFi{
 ...
 ~~~~ \newFi{
   \inference{\Delta |- F=F':A->\kappa & \Delta;\cdot |- s=s':A}
             {\Delta |- F\,\{s\} = F'\,\{s'\} : \kappa} }
\]
\[ \inference{|- \kappa & \Delta,X^\kappa |- B=B':*}
...
\[ \inference{\Delta;\Gamma |- t=t':A}{\Delta;\Gamma |- t'=t:A}
 ~~~~
   \inference{\Delta;\Gamma |- t=t':A & \Delta;\Gamma |- t'=t'':A}
             {\Delta;\Gamma |- t=t'':A}
\]
\[ \inference{|- \kappa & \Delta, X^\kappa;\Gamma |- t=t' : B}
...
\begin{proposition}[well-formed type level context ensures well-sorted kinding judgement]
\[ \inference{ |- \Delta & \Delta |- F : \kappa}{ |- \kappa:\square }
\]
\end{proposition}
\begin{proof}
Mutual induction on the derivation on the kinding judgement
and the typing judgement \KYA{since proof of this proposition and the next
proposition depends on each other, but I'm a little bit confusing what
exactly I need to induct on.}
\begin{itemize}
\item[case] ($Var$)
...
\item[case] ($\lambda$)
	By induction and the second well-formedness rule of $\Delta$
\item[case] ($@$)
	By induction.
\item[case] ($\lambda i$)
	By induction and the third well-formedness rule of $\Delta$.
\item[case] ($@i$)
	By induction provided that we can prove the next proposition that
	well-formed term level context ensures well-kinded typing judgement.
\item[case] ($->$)
