...
with Curry style terms and typing contexts sparated into two zones
(\ie, type level and term level). Temrs are curry style. That is,
...
Typing contexts are separted into type level contexts ($\Delta$) and
...
the index arrow kind $A -> \kappa$ must be well kindined under
...
We have type level contexts ($\Delta$) for type level bindings
...
A type level context $\Delta$ is well-formed when it is either empty, extended by
...
well-kinded under the empty type level context. Similarly to the well-sorted rule
($Ri$) for indexed arrow kinds, we require $A$ of $i^A$ to be well-kinded in
the empty type level context ($\cdot$), since no bindings are introduced
at kind level in \Fi.

A term level context $\Gamma$ is well-formed under a type level contexts $\Delta$
...
\begin{proposition}[well-formed type level context ensures well-sorted kinding judgemnet]
