...
\\
\!\!\!\!\!\!\!\!&\text{Terms}
	& r,s,t			&~ ::= ~ x \mid i \mid \lambda x.t \mid r\;s
\\
\!\!\!\!\!\!\!\!&\text{Contexts}
	& \Delta		&~ ::= ~ \cdot
				\mid \Delta, X^\kappa
...
\[  \inference{}{|- \cdot}
 ~~~~
   \inference{|- \Delta & \Delta |- \kappa:\square}
      ...
      \big( X\notin\dom(\Delta) \big)
 ~~~~ \newFi{
   \inference{|- \Delta & \cdot;\cdot |- A:*}
             {|- \Delta,i^A}
      \big( i\notin\dom(\Delta) \big) }
\]
\[ \inference{|- \Delta}{\Delta |- \cdot}
 ~~~~
   \inference{\Delta |- \Gamma & \Delta;\cdot |- A:*}
             {\Delta |- \Gamma,x:A}
      \big( x\notin\dom(\Gamma) \big)
\]
...
Since we have index polymorphic types ($\forall i^A . B$),
...
\[
   \inference[($\lambda i$)]
      { \inference[(@i)]{ \Delta, i^A |- F : A -> \kappa
                        & \inference[($:'$)]{ i^A\in\widehat{\Delta,i^A}
                                            & |- \widehat{\Delta,i^A} }
                                            {\widehat{\Delta,i^A} |- x:A}
                        }
                        {\Delta, i^A |- F\{x\} : \kappa} }
      {\Delta |- \lambda i^A.F\{x\} :\kappa}
\]
\KYA{we stil need to show a small lemma that context lifting preserves
well-formedness of cotnexts so that we can accept $|-\widehat{\Delta,i^A}$
assuming $|-\hat\Delta$. TODO put this in the next paragraph.}

\paragraph{Contexts}
We have a forms of bindings $i^A$, which we call index binding,
...
\newFi{i^A} is an index binding, which can only be used statically
(\ie, at type level) but not dynamically (\ie, at term level).
Index bindings ($i^A$) are introduced either from index abstractions
($\lambda i^A.F$) or from index polymorphic types ($\forall i^A.B$).
...
$\Delta,i^A |- F \{x\} :*$, since the body of index abstraction ($F\{x\}$)
is at type level. However, the index biniding $i^A$ introduced by
...
and leaves the others ($X^\kappa$ and $i^A$) unchaged.
