...
Before go on we prove subjected reduction and strong normalization,
we need to check some basic well-fromeness properties of the sorting, kinding,
and typing rules of \Fi\ such that dervations under well fromed contexts
entail well-formed results. More specifically, we want to make sure that
three propositions hold: sotring derivations entail well-formed sorts
(Proposition \ref{prop:wfsort}), kinding derivations under well-formed
type level contexts entail well-sorted kinds (Proposition \ref{prop:wfkind}),
and typing derivations under well-formed type and term level contexts entail
well-kinded types (Proposition \ref{prop:wftype}). Techincally, to be rigourous
about the induction principle for the proof, these three properties should be
considered as projections of one a big conjucted property -- a derivation
for a judgment of either sorting, kinding, or typing under well-formed contexts
entail either a well-formed sort (when it is a sorting judgment),
a well-sorted kind (when it is a kinding judgment), or
a well-kinded type (when it is a typing judgment) -- since the definition
of sorting, kinding, and typing rules are mutually recursive. Thus, we can
prove this well-formeness property of \Fi\ by induction on the derivation
of a judgment, which can be either one of the three forms. Here, I illustrate
the proof for the three propositions as if they were sepreate proofs mutually
referencing each other, since they are easier to understand that way. But,
techicall, when I say ``by induction'' during the proofs of these propositions,
what I really mean is the induction hypothesis of that big conjucted property.

\begin{proposition}[sorting derivations entail well-formed sorts]
\label{prop:wfsort}
$ \inference{ |- \kappa : \mathfrak{s}}{ \mathfrak{s}=\square } $
\end{proposition}
\begin{proof}Obvious since $\square$ is the only sort in \Fi.\end{proof}

\begin{proposition}[kinding derivations under well-formed type level context
	       	entails well-sorted kinds]
\label{prop:wfkind}
...
\item[case] ($Conv$)
	By induction and a helper lemma that kind equality rules ensure
	that both sides of equality are well-sorted. This lemma should
	be straightforward to prove, but this lemma also is needs
	mutual induction with the lemma that type equality rules ensure
	that both sides of equality are well-kinded.
\item[case] ($\lambda$)
	By induction and Proposition \ref{prop:wfsort} we know
	that $|- \kappa:\square$.
	By the second well-formedness rule of $\Delta$,
	we know that $|- \Delta,X^\kappa$
	since we alredy know that $|- \kappa:\square$ and $|- \Delta$.
	By induction, we know that $|- \kappa':\square$
	since we already know that $|- \Delta,X^\kappa$ and
	we know that $\Delta,X^\kappa|- F:\kappa'$ from induciton hyphothesis.
...
\item[case] ($\lambda i$)
	...
	that $\cdot|- A:*$ since we already know that $|- \Delta$.
	By the third well-formedness rule of $\Delta$,
	we know that $|- \Delta,i^A$
	since we alredy know that $\cdot|- A:*$ and $|- \Delta$.
	By induction, we know that $|- \kappa:\square$
	since we already know that $|- \Delta,i^A$ and
	we know that $\Delta,i^A|- F:\kappa$ by induction hypothesis.
...
\begin{proposition}[well-formed term level context ensures well-kinded typing judgement]
\label{prop:wftype}
\[ \inference{ \Delta |- \Gamma & \Delta;\Gamma |- t : A}{ \Delta |- A : * }
\]
\end{proposition}
\begin{proof}
The same induction principle as the previous proposition since the proof
of these propositions are mutually recursive.
\begin{itemize}
\item[case] ($:$)
...
\item[case] ($=$)
	By induction and a helper lemma that type equality rules ensure
	that both sides of equality are well-kinded. This lemma should
	be straightforward to prove, but mutually inductive proof with
	the lemma on kind equality rules, which we used in the proof of
...
\item[case] ($\forall E$)
	TODO some sort of substitution lemma?
\item[case] ($\forall Ii$)
	By induction and well-formedness of $\Delta$.
\item[case] ($\forall Ei$)
	TODO some sort of substitution lemma?
\end{itemize}
\end{proof}
\KYA{After going through the proof sketch for the two propositions above,
I relaized that the rules for \Fi\ needs more well-typeness and well-kindness
conditions (\eg, in ($\forall I$) and ($\forall Ii$)), unlike \Fw.
We need some adjustment on the \Fi\ rules.}
