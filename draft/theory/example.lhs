...
Mendler observed \KYA{well, did Mendler?} that each operator can be defined by
the equation at the bottom of its figure. Each operation can be given a naive
type involving (|Mu[*] T|), but if we instead give it a more abstract type,
abstracting values of type (|Mu[*] T|) into some unknown abstract type |r|, 
one can safely guarantee a certain pattern of use that insures termination.  
Informally, if the operation works for some unknown type |r| it will certainly
...
there is a more expressive Mendler style operation that is safe over
recursive types with negative operations. We call this operator |MsfIt|.
...
The next few chapters formalize the theory behind Nax. We want Nax to be
a sound and consistent logic. In the future we want Nax programs
to include both a logical fragment, and a non-logical (or programatic) fragment,
