...
\[
	...
	\l f^{B\to A}.\, \l i^{A}.\, \forall X^{*}.\,
	  (\forall j^B.\, \LEq_A\s i\s{f\,j}\to X) \to X
\enspace.
\]
(Intuitively, this encodes an existential quantifier as in the informal
type ``$\l f^{B\to A}.\, \l i^{A}.\, \forall X^{*}.\, (\exists j^B.\,
\LEq_A\s i\s{f\,j})\to X \to X$''.)
Note that
\[
	\l h.\, h(\l x.\,x)
	:
...
=
\l i^{\mathtt{Nat}}.\,\forall X^{*}.\, 
(\forall j^{\mathtt{Nat}}.\, \LEq_{\mathtt{Nat}}\s{i}\s{\mathtt{S}\,j}\to X) 
\to X 
\]
allows us to define 
\[
