...
Left Kan extensions come with a canonical function of type
$\forall f^{A\to B}.\,\forall X^{A\to *}.\,\forall i^A.\, X\s i \to
(\Lan_{A,B}\s f X)\s{f\,i}$ that, according to a reindexing function
$t:A\to B$, transforms an \mbox{$A$-indexed} type $F$ (at index $s$) into
the $B$-indexed type $\Lan_{A,B}\s t F$ (at index $t\,s$).  This can be
used, for instance, to embed one-dimensional arrays into two-dimensional
matrices or, conversely, to project a two-dimensional matrices down to
