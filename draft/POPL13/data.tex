...
-- below is Haskell-ish pseudo code
...
\end{array}\]
are inhabited, and hence that Leibniz equality is a congruence.
%\[\begin{array}{l}
...
construction is given by the type constructors
$\Ext_{A,B}: (A\to B) \to (A\to*) \to B\to *$, which are in spirit right
Kan extensions~(\eg~\cite{AbeMatUus03}), defined as 
\[
\Ext_{A,B}
\triangleq
\l f^{A\to B}.\,
...
so that 
\[
\mathtt{FinZ}\,(\lambda x.\, x): \mathtt{Fin}\s{\mathtt S\, t}
\enspace,\quad
\mathtt{FinS}:
...
With $\mathtt{Fin}$, one defines the type of \mbox{$\lambda$-terms} in
context 
%$\mathtt{Lam}:\mathtt{Nat}\to*$
as:
\[
%\mathtt{Lam} \triangleq
%\!\!\!
...
\\[1mm]
\quad
  (\forall j^\mathtt{Nat}.\,\mathtt{Fin}\s j \to X\s j)
\\[1mm]
\quad\quad
