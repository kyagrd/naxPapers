...
$\mathtt{S}=\l x.\l x_s.\l x_z.x_s(x\,x_s x_z)$ and
$\mathtt{Z}=\l x_s.\l x_z.x_z$.
...
successor and zero states that: to eliminate $\mathtt{S}\,x$, apply $x_s$
to the elimination of the predecessor $(x\,x_s x_z)$; and,
to eliminate $\mathtt{Z}$, just return $x_z$.
...
For $F_A=\Eq_A,\LEq_A$, one has $\Delta\vdash F_A \s s \s t =
F_A\s{s'}\s{t'}:*$ for all $\Delta;\cdot\vdash s=s':A$,
$\Delta;\cdot\vdash t=t':A$.
%
As a further basic property, one also has that the types 
\[\begin{array}{l}
...
\end{array}\]
are inhabited.  Hence Leibniz equality is a congruence; in that, in
addition to the above, the type
\[
\forall i^A.\,\forall j^A.\,
  \Eq_A\s{i}\s{j}\to\Eq_A\s{j}\s{i}
\]
is inhabited.  This is not so for the type 
\begin{equation}\label{NonSymmetry}
\forall i^A.\,\forall j^A.\, \LEq_A\s{i}\s{j}\to\LEq_A\s{j}\s{i}
\enspace.
\end{equation}
(Cf.~Examples~\ref{PathologicalExample}
and~\ref{PathologicalExampleContinued} in \S\ref{sec:theory}.)
...
(A\to*) \to B\to *$ (which are in spirit right Kan extensions, see
