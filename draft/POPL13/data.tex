...
\end{array}\]
is thus inhabited by a retraction, coercing (forth and back) between the
two functional representations.

%\begin{comment}
\begin{example}
One can give a version of the constantly unit type 
\[
	(\l i^{A}.\, \forall Z^{*}.\,Z\to Z) : A\to *
\]
that is restricted according to an index test function.  This is done as
follows 
\[
	\mathtt T_{A,B} \triangleq
	\l f^{B\to A}.\, \l i^{A}.\, \forall Z^{*}.\,
	  (\forall j^B.\, \LEq_A\s i\s{f\,j}\to Z) \to Z
\enspace.
\]
(Intuitively, we are encoding a double-negated existential type as in the
informal type ``$\l f^{B\to A}.\, \l i^{A}.\, \mathtt K(\exists j^B.\,
\LEq_A\s i\s{f\,j})$'' for $\mathtt K\triangleq \lambda X^{*}.\,\forall
Z^{*}.\,(X\to Z)\to Z$ the
codensity monad.)
Note that
\[
	\l h.\, h(\l x.\,x)
	:
	\forall f^{B\to A}.\,\forall j^{B}.\,
	(\mathtt{T}_{A,B}\s{f})\s{f\, j}
\]
so that, in particular,
\[
	\l h.\, h(\l x.\,x)
	:
	\forall i^{A}.\,(\mathtt{T}_{A,A}\s{\l x.\,x})\s{i}
	\enspace.
\]

For the example
\[
\mathtt T_{\mathtt{Nat},\mathtt{Nat}}\s{\mathtt S}
=
\l i^{\mathtt{Nat}}.\,\forall Z^{*}.\, 
(\forall j^{\mathtt{Nat}}.\, \LEq_{\mathtt{Nat}}\s{i}\s{\mathtt{S}\,j}\to Z) 
\to Z 
\enspace,
\]
the type $\forall i^\mathtt{Nat}.\,(\mathtt
T_{\mathtt{Nat},\mathtt{Nat}}\s{\mathtt S})\s{\mathtt S\,i}$ is inhabited,
while $(\mathtt T_{\mathtt{Nat},\mathtt{Nat}}\s{\mathtt S})\s{\mathtt Z}$
is not.
%
Encode thus
\begin{verbatim}
...
\quad 
(\forall j^\mathtt{Nat}.\,
(\mathtt T_{\mathtt{Nat},\mathtt{Nat}}\s{\mathtt S})\s{j}
     \to X\s j)
\\[1mm]
...
& : & 
  \forall i^{\mathtt{Nat}}.\,
  (\mathtt T_{\mathtt{Nat},\mathtt{Nat}}\s{\mathtt S})\s{i}
    \to \mathtt{Fin}\s i
\\[1mm]
\l h.\, \l f.\, \l g.\,
  g\, (h\, f\, g)
...
\[
\l f.\,\l g.\, f\,(\l x.\, x)
: \forall i^\mathtt{Nat}.\,\mathtt{Fin}\s{\mathtt S\, i}
\enspace.
\]
\end{example}
