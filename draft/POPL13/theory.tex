...
\begin{itemize}
\item[case] ($:$)
By Theorem \ref{thm:ierasetmctx}, we know that $\Delta^\circ|- \Gamma^\circ$
when $\Delta|- \Gamma$.
By definition of erasure on term-level context, we know that
$x:A^\circ \in \Gamma^\circ$ when $x:A \in \Gamma$.
\item[case] ($:i$) Vacuously true since we assume that $t$ does not
	contain any index variables.
By Theorem \ref{thm:ierasetyctx}, we know that $|- \Delta^\circ$
when $|- \Delta$.
By definition of erasure on term-level context $\Delta$, we know that
$x:A^\circ \in \Gamma^\circ$ when $x:A \in \Gamma$.
...
\]
Consier when $A=Void$ and $B=\forall i^{Void}.NeverEverVoid\,\{i\}$.
