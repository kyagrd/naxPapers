...
\item[case] ($=$)
	By Theorem \ref{thm:ierasetyconeq} and induction.\qed
\end{itemize}
\end{proof*}
...
full type dependency, see also \S\ref{NoVoid}.

Similar considerations to the above show that $\LEq_A$ is not symmetric,
in that the type of~(\ref{NonSymmetry}) in~\S\ref{Leibniz} is not
inhabited.  
\end{example}

We introduce an index variable selection operation~($^\bullet$) that
selects all the index variable bindings from the type level context.
