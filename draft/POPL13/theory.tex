...
\S\ref{sec:motiv} is $\mathtt{List} \triangleq \lambda
A^\mathtt*.\,\forall X^\mathtt*.\, X \to (A\to X\to X)\to X:
\mathtt*\to\mathtt*$.
%
However, one can build pathological examples.  To this end, note that the
type $\forall i^A.\,\forall j^A.\, \LEq_A\s i\s j$ has index erasure
$\mathtt{Unit} \triangleq \forall X^\mathtt{*}.\,X\to X$.
\end{example}
...
that the implication of the above theorem does not admit a converse.
\end{remark}
