...
while retaining the type structure.  
%
This is clearly seen for the vector type 
constructor~$\mathtt{Vec}: \mathtt * \to \mathtt{Nat} \to \mathtt*$ of
\S\ref{sec:motiv} whose index erasure is the list type
constructor~$\mathtt{List} \triangleq \lambda A^\mathtt*.\,\forall
X^\mathtt*.\, X \to (A\to X\to X)\to X: \mathtt*\to\mathtt*$.
%
One can however build pathological examples.  For instance, 
the type 
$\mathtt P_A\triangleq\forall i^A.\,\forall j^A.\, \LEq_A\s i\s j$ has
index erasure $\mathtt{Unit} \triangleq \forall X^\mathtt{*}.\,X\to X$.
\end{example}
...
We apply the theorem to observe that the pathological type~$\mathtt P_A$
...
It follows as a corollary that the implication of the above theorem does
not admit a converse.
...
full type dependency.

Similar considerations to the above show that $\LEq_A$ is not symmetric,
in that the type $\forall i^A.\,\forall j^A.\, \LEq_A\s i\s j\to \LEq_A\s
j\s i$ is not inhabited.
\end{example}
...
\subsection{No void type instantiation from dynamic values}
...
the instantiation of uninhabited type. Consider the instantiation rule of ICC,
shown below:
\[
...
provided that $(y:Void)\in\Gamma$. Note that, in ICC, it is possible to
instantiate a universally quantified term variable $i$ of an uninhabited type
from a possibly dynamic term $x$.

In \Fi, we cannot instantiate $B$ with any of the term variables since
index instantiation can not refer to the term-level context ($\Gamma$)
but only refers to the type-level context ($\Delta$).
Recall the ($\forall E i$) rule:
\[
	\inference[($\forall E i$)]{ \Delta;\Gamma |- t : \forall i^A.B & \Delta;\cdot |- s:A }
	                           {\Delta;\Gamma |- t : B[s/i]}
\]
...
type constructor definition, as in Example~\ref{PathologicalExample}.
Void type instantiation is localized inside type constructor definition.
It is assured that function definitions at term level will never cause
void type instantiation, even when some of the function arguments have
uninhabited type or pathological type.
