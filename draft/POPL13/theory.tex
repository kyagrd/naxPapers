...
\mathtt*\to\mathtt*$.
%
However, one can build pathological examples.  To this end, let $\mathtt0
\triangleq \forall X^\mathtt{*}.\,X$ and consider %the type 
$\forall F^{\mathtt{0}\to *}.\,\forall i^\mathtt{0}.\,\forall
j^\mathtt{0}.\, F\{i\}\to F\{j\}$.  This uninhabited type has index
erasure $\mathtt1 \triangleq \forall X^\mathtt{*}.\,X\to X$.
\end{example}
