...
much difference. {\bf I don't understand the previous sentence. Tim}.
Notationally, we use $A$ and $B$, instead of $F$ and $G$, where
$A$ and $B$ to are expected to be types (\ie, nullary type constructors)
of kind $*$.


In a language with term indices, terms appear in types (i.e. the length index $(n+m)$ in the type $Vector Int \{n+m\}$).
Such terms contain variables. The binding sites of these variables matters.
In \Fi\ we expect such variables to be statically bound. Dynamically bound
index variables would require a dependently typed calculus, such as the calculus of constructions.
To reflect this design choice, typing contexts are separated into type level contexts ($\Delta$) and
...
term variables to be used as indices ($i$). Our design choice, to write $i$, instead of $x$,
when term variables are to be used as indices (\ie, introduced by
either index abstraction or index polymorphism) is a matter of convenience only. We could have
used a single syntactic set for both -- the theory would not change. Context can always
be used to distinguish the two uses. But, the choice of two distinct syntactic sets, makes
terms easier to read. For similar reasons, we surround terms in types with braces ($\{ ... \}$)
to visually distinguish the transition from type to term.


In contrast to our design choice, System \Fw\ is most often formalized using a single context, which
...
term variables used in types (which we call index variables, or indices, and are bound as ($\Delta,i^A$)) from the ordinary use of
term variables (which are bound as $\Gamma,x : A$). The expectation is that indices
...
introduced at kind level in \Fi. {\bf I don't understand this. Tim}.
...
described in Paragraph {\it {\bf Sorting}} 
for term indexed arrow kinds. The consequence of
...
the type generalization rule ($\forall I$), except for the additional
side condition $i\notin\FV(t)$. {\bf SOMETHING is wrong here. It says $\Gamma$ not t in the rule. Tim.} 
This side condition prevents
terms from accessing the type level index variables introduced by index
polymorphism. Without this side condition, $\forall$-binder
