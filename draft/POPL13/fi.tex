...
Type constructors remain Church style. That is, type level abstractions are
...
term variables to be used as indices ($i$). We write $i$, instead of $x$,
...
However, in \Fi, we spearated the context into two parts to distinguish
...
Index variables are bound in the type level context ($\Delta,i^A$),
...
We write $x$ for ordinary term variables introduced from
term level abstractions ($\lambda x.t$).
We write $i$ for index variables introduced from
index abstractions ($\lambda i^A.F$) and
index polymorphic types ($\forall i^A.B$). The distinction between
$x$ and $i$ is just a convention for the convenience of readability.
...
side condition $i\notin\FV(t)$. This side condition prevents
