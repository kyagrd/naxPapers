...
(or, proof assistants). By the Curry--Howard correspondence, it is possible,
in principle, to design a unified language based on a typed lambda calculus for
both logical reasoning and programming. However, different requirements for
programming language and reasoning systems makes it difficult to design such
a unified language useful both purposes. Programming languages usually extend
lambda calculi with programming friendly features (\eg, recursive datatypes,
general recursion), which provide flexibility to model various computations,
while sacrificing logical consistency. Logical reasoning systems extend lambda
calculi with logic friendly features (\eg, induction principles, dependent
types), which grantee paradox-free inference over fine-grained properties,
while being more restrictive in modeling computations.

In this dissertation, we design a language called Nax, which balances
between the benefits of the both. Nax accepts all recursive datatypes,
allowing the same flexibility of defining recursive datatypes
just as in functional programming languages. Nax supports a number of
Mendler-style recursion schemes that can express various kinds of recursive
computations and also grantee termination. Nax supports term-indexed types
to enable specifications of fine-grained properties. In addition, Nax supports
a conservative extension of Hindley--Milner type inference.

Theoretical contributions of this dissertation are the theories for
...
for their useful feedback, and special thanks Gabor Grief, who gave feedback on
my thesis related research before even asking. Many thanks to my parents,
