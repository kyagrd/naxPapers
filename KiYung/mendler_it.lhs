...
\section{Defining regular recursive datatypes} \label{ssec:tourRegular}
...
in general recursion style (left) and in Mendler style (right).
\begin{center}
...
\end{minipage}
\end{center}
We define |List p| as |Mu0 (L p)|, which is the fixpoint of
...
is defined on the very same fixpoint, |Mu0|, as in Mendler style,
provided that the base datatype |f| is a functor.
This, more widely known approach \cite{hagino87phd},
was independently developed at about the same time as Mendler style.
...
the list length function |len| in general recursion style
on the left-hand side of Figure \ref{fig:len}.
\begin{figure}
...
as |foldr|. This conventional iteration is more often used than
...
base type must be a functor, but still maintains the strict termination
...
decreases the number of |In0| constructors in its argument.\footnote{
	We assume that the values of recursive types are always finite.
	We can construct infinite values (or co-recursive values) in Haskell
	by exploiting laziness, but we exclude such infinite values from
	our discussion in this work.} 

The first property is easy to observe from the definition of |mcata0|
in Figure \ref{fig:rcombdef}, particularly the pattern matching of
...
we name the first argument of |phi|, which is the recursive caller, as |len|.
We use this |len| exactly where we would recursively call the recursive function
in general recursion style (|len| on the left).

However, unlike general recursion style, it is not possible to call
...
while |(C x xs) :: L p r| has a more specific type than |r|.
