...
The embeddings of $\unIn_{(R A)}$ relies on \textit{fmap} over \texttt{List}s.
The idea is to \textit{cast} all the abstract recursive subcomponents of type
$X_r$ inside $F\;X_r$ values into a concrete $\mu_{*}^{+}F$ in order to obtain
$F(\mu_{*}^{+}F)$ values. The $\textit{fmap}_\texttt{List}$ function applies
\textit{cast} to each of the the abstract recursive subcomponents of type
$X_r$ inside $F\;X_r$ values into a concrete recursive type $\mu_{*}^{+}F$
in order to obtain $F(\mu_{*}^{+}F)$ values. 
...
in Figures~Figure~\ref{fig:HaskellFunctor1}, \ref{fig:HaskellunInRegular},
and \ref{fig:HaskellunInNested}.

We provide Haskell transcriptions of the unroller embeddings for
regular datatypes in Figure~\ref{fig:HaskellunInRegular}
(Haskell definitions of \texttt{Mu0} and \texttt{mcvpr0} are
in Figure~\ref{fig:HaskellFunctor1}).
Note the definitions of all these unrollers are uniform:
...
So, we can derive a uniform unroller embedding for any base $F:+* -> *$
as follows:
\begin{align*}
...
We provide Haskell transcriptions of the unroller embeddings
for nested datatypes in Figure~\ref{fig:HaskellunInNested}.
we provide Haskell transcriptions of $\unIn_P$ and $\unIn_B$
in Figure~\ref{fig:HaskellunInNested}
(Haskell definitions of \texttt{Mu1} and \texttt{mcvpr1} are
in Figure~\ref{fig:HaskellFunctor1}).

To embed unrollers for term-indexed datatypes, we would often need
existential types (Figure~\ref{fig:fixiNonRecData}) and equality types.
We can encode equality types in \Fixi\ as a Leibniz equality over indices,
\ie, $(i=j) \triangleq \forall F^{A -> *}.F\{i\} -> F\{j\}$, as discussed
in \S\ref{Leibniz}.


Also, note that
the definition of \texttt{unInP}
and \texttt{unInB}
in Figure~\ref{fig:HaskellunInNested} are of the same form:
...
\emph{truly nested datatype} \cite{AbeMatUus05}, is not constant time either
