...
System \F\ extends the type syntax of the STLC with type variables ($X$)
...
for ground types, such as the void type $\iota$ in the STLC.
In System \F, we can populate types from forall types such as $\forall X.X$.
This type is, in fact, an encoding of the void type. We shall see that
a large class of datatypes are encodable in System \F (\S\ref{sec:f:data})
...
pretty much the same as in the STLC except that we carry around the kinding context
