...
& \inference[\sc App]{\Delta;\Gamma |- t : A -> B & \Delta;\Gamma |- s : A}
		     {\Delta;\Gamma |- t~s : B} \\
& \inference[\sc TyAbs]{\Delta,X;\Gamma |- t : B}
		       {\Delta;\Gamma |- t : \forall X.B \phantom{\L X}} ~
		       (X\notin\ran(\Gamma)) \\
& \inference[\sc TyApp]{\Delta;\Gamma |- t : \forall X.B & \Delta |- A}
		       {\Delta;\Gamma |- t : B[A/X]}
\end{align*}
...
\rulename{RedTyAbs} and \rulename{RedTyApp} in Church style correspond to
...
\item[Interpretation of terms] as terms themselves whose free variables are
	substituted according to the given pair of valuations ($\xi$;$\rho$):
\begin{align*}
[| x      |]_{\xi;\rho} &= \rho(x) \\
...
The Proof of strong normalization amounts to proving
\[ \inference{\Delta;\Gamma|- t:A & \xi;\rho \in [|\Delta;\Gamma|]}
		{[|t|]_{\xi;\rho} \in [|A|]_\xi}
	\quad \text{soundness of typing}
\]
The details of the proof is illustrated in Figure \ref{fig:SNproofF}.
...
interpretation such that $\xi(X)=X$ and $\rho(x) = x$ for all the free variables. Note that
...
\end{corollary}
\caption[Strong normalization proof for System \F]{
	A proof of strong normalization for System \F\ via proving
