...
~\\
\begin{tabular}{llll}
	& encoding of type & constructors & eliminators
	\\
...
boolean & $\forall X.X -> X -> X$
	\\

natural
number	& $\forall X. X -> (X -> X) -> X$
	\\

list	& $\forall X_a.\forall X. X -> (X_a -> X -> X) -> X$
	\\

pair	& $\forall X_1.\forall X_2.\forall X. X_1 -> X_2 -> X$
	\\

sum	& $\forall X_1.\forall X_2.\forall X. (X_1 -> X) -> (X_2 -> X) -> X$
\end{tabular}
