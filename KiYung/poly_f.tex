...
All of these datatypes are classified as \emph{regular datatypes}.\footnote{
	We will discuss regular datatypes
...
as in Table \ref{tbl:dataF}.
%%% carry over to aaaaaaa just about the terms
%%% F well typed F
Such encoding of types in System \F\ for
...
type applications in appropriate places. %%% example 

Constructors produce objects of a datatype. Nullary constructors
(\aka\ constants) are objects by themselves. For example, %% actual incondings
...
the eliminator for void (TODO) expects only one argument since void has no constructor,
the eliminator for unit (TODO) expect two arguments since unit has one constructor, and
...
For constants, the eliminator simply returns the argument being passed to handle
...
The key idea of Church encoinding is that objects are be defined by
