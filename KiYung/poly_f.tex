...
in Church style.\footnote{
	The corresponds of reduction steps is not always one step to one step
	since the reduction rules {\sc RedTyAbs} and {\sc RedTyApp}
...
it also holds for $\xi'=\xi$ and $\rho'=\rho$. Thus,
\[ [|t|]_{\xi;\rho} \in [|\forall X.B|]_{\xi}
        = \bigcap_{\mathcal{A}\in\SAT} [|B|]_{\xi[X\mapsto\mathcal{A}]}
