...
in Mendler style using an indexed base datatype.
...
The general recursive version on the left-hand side of Figure~\ref{fig:mutrec}
...
|DecExpF| include the data constructors of declarations (|Def|) and
expressions (|Var|, |Val|, |Add|, and |Let|).  The data constructor for
declarations is indexed by |D| and the other data constructors for
...
You can observe that the definition of |phi| is very close to the definitions
of the general recursive versions of |extend| and |eval| on the left.
The difference is that we project out |ev| from |f|, which is the handle for
the combined mutually recursive function, when we need to call the evaluation
