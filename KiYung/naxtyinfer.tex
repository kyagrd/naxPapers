...
typing rules of the form \framebox{$\Delta;\Gamma |-s t : A$}, which
assignes a type to a monomorphic type ($A$), the rule \rules{Alt$_s$}
of the form \framebox{$\Delta;\Gamma |-s^\psi C\overline{x} -> t : \sigma$}
...
%% TODO decl is sound w.r.t Fw
Declarative typing rules of SmallNax (without the features for datatypes)
...
excluding the rules for datatypes (\rulename{TCon}, \rulename{Con},
\rulename{Case}, \rulename{Alt}), in Figure\;\ref{fig:SmallNax} are
...
%% TODO give an idea why we can make an algorithm
