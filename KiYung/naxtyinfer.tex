...
%% and it becomes more difficult when the language
%% supports wider range of indexed datatypes.
...
We need additional polymprhism in the typing rule for \MIt\ when we have free
variables in the index transformer ($\psi(\overline{X})$), other than
the indices ($\overline{X}$) of the argumnet type. For exmaple, consider
the index transforemr $\{\{t\}\,.\,
   \texttt{Code}\,\{\textit{ts}\}\,\{\texttt{`cons}\;t\;\textit{ts}\}\}$
appearing in the definition of the stack-safe compiler (\textit{compile})
in \S\ref{ssec:compile}. The free varaible (\textit{ts}) should be generalized
...
should be fully genernalized, except for the part of the input type
