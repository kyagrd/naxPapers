...
The syntax and typing rules of the Hindley-Milner type system (HM) are
illustrated in Figure \ref{fig:hm}. The details of the sindax is described
in \S\ref{sec:hm:syntax}. There are two sets of typing rules.
The declarative typing rules (\S\ref{sec:hm:dectyrule}) are suited for
reasoning about the soundness of typing.
The syntax directed typing rules (\S\ref{sec:hm:dectyrule}) are suited
...
the same as the terms in the Curry-style System \F, considering the let-term
as a syntactic sugar. Types and type schemes in HM are restrictions of types
in System \F. The declarative typing rules are also a restriction of
...
A derivation step of \rulename{Let} in HM can be translated into two steps of
derivation involving the \rulename{App} and \rulename{Abs} rules in System \F,
...
\[\inference[\sc App]
	...
	{\Delta;\Gamma |- (\l x.t)\;s}
\]
A derivation step of \rulename{Inst} in HM can be translated into multiple
stpes involving the \rulename{TyAbs} and \rulename{TyApp} rules in System \F,
as below:
{\small
\begin{multline*}
  \inference[\sc Inst]{\Gamma |- t : \sigma & \sigma \sqsubseteq \sigma'}
