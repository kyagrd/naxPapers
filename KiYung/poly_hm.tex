...
instantiation is a special case of generic instantiation. For example,
...
such as the identity function $\l x'.x'$. We want bind this function
...
The declarative typing rules are also a restriction of the typing rules
...
an abstraction $(\ x.t)$ to the body of the local definition ($s$).
...
The Curry-style System \F\ have the rules, which exactly captures the concept
of those small steps (see Figure \ref{fig:f} in \S\ref{sec:f}).
The \rulename{TyApp} rule captures the small step in the instantiation part.
The \rulename{TyAbs} rule captures the small step in the generalization part.
Therefore, we can translate the \rulename{Inst} rule in HM into
a consecutive applications of the \rulename{TyApp} rule followed by
a consecutive applications of the \rulename{TyAbs} rule in System \F, as below:
{\small
\begin{align*}
& \inference[\sc TyApp]
...
($\Gamma |- t : A$). These rules are syntax directed since there is
only one rule to apply for each syntactic category of terms.
...
We will just lay out a key idea for the proof of the soundness of $|-s$
...
The completeness of $|-s$ is stated as the theorem below.
Note that the completeness of $|-s$ is to be stated in terms of closure and
type scheme ordering relation ($\overline{\Gamma}(A)\sqsubseteq\sigma$),
since the syntax directed rule can only deduce types, not type schemes.
The syntax directed rule $|-s$ is complete in the sense that for any given
...
\end{theorem}
\begin{proof}
We proof by induction on the derivation $\Gamma |- t : \sigma$.
Let us consider cases by the last rule (\ie, root of the derivation).
...
which neither appears free in $\sigma$ nor $\Gamma$. Then such the closure
of such $A$ satisfy $\overline{\Gamma}(A)\sqsubseteq\sigma$.
...
We would be done with the case for \rulename{Let} if we can show that
$\Gamma,x:\overline{\Gamma}(A') |-s t : B' \land
	\overline{\Gamma,x:\overline{\Gamma}(A')}(B') \sqsubseteq B$.
instead of $\Gamma,x:\sigma |-s t : B' \land
	\overline{\Gamma,x:\sigma}(B') \sqsubseteq B$.
We use Lemma \ref{lem:generalizeGamma} to show
$\Gamma,x:\overline{\Gamma}(A') |-s t : B'$ from $\Gamma,x:\sigma |-s t : B'$,
and we use Lemma \ref{lem:closureMoreGen} to show
$\overline{\Gamma,x:\overline{\Gamma}(A')}(B') \sqsubseteq B$
from $\overline{\Gamma,x:\sigma}(B') \sqsubseteq B$ and transitivity of
$\sqsubseteq$. These two lemmas are introduced after the proof of this throem.
...
that $X \notin\FV \overline{\Gamma}(A)$ by definition of closure --
if it were $X\in\overline{\Gamma}(A)$, then it should be $X\in\FV(\Gamma)$
by defintion of closure, which contradicts the side condition of
the \rulename{Gen} rule $X\notin\FV(\Gamma)$.
Recall that generic instantiation allows quantifiying type variables
that do not apper free the original type scheme. Thus,
...
by transitivity of $\sqsubseteq$ on type schemes.
...
By \rulename{Var$_s$} rule, we can dedece any $A'$ for $x$
...
It is easy to see that $\FV(\sigma')\sqsubseteq\FV(\sigma)$
when $\sigma' \sqsubseteq \sigma$. Since the defintion of $\sqsubseteq$
over typing contexts only depends on $\sqsubseteq$ on type schemes,
