...
The type inference algorithm $W$ (Figure \ref{fig:algW}) is sound
with respect to the syntax directed typing rules (Theorem \ref{thm:soundW}).
...
\item[case]($s\;t$)
	We want to show that \vspace*{-2em}
	\begin{singlespace}
	\[\inference{S'\Gamma |-s s\;t : B}{
	\begin{matrix} W(\Gamma,s\;t) ~\qquad~ \\
	...
	\vspace*{-2em}
	\begin{singlespace}
	\[\inference{S''((S_3\circ S_2\circ S_1)\Gamma) |-s s\;t : B'}{
	\begin{matrix} W(\Gamma,s\;t) ~\qquad~ \\
		~> (S_3\circ S_2\circ S_1,S_3 X) \end{matrix}
	~\land~
	\exists R.
	...
	\inference{S_1'\Gamma |-s s : A' -> B'}{
	\begin{matrix} W(\Gamma,s) \\ ~> (S_1,A_1) \end{matrix}
	~\land~ 
	\exists R_1.
		\left(\begin{matrix}
	...
	\inference{S_2'(S_1\Gamma) |-s t : A'}{
	\begin{matrix} W(S_1\Gamma,t) \\ ~> (S_2,A_2) \end{matrix}
	~\land~
	\exists R_2.
		\left(\begin{matrix}
	...
	\begin{singlespace}
	\begin{align*}
	\inference{S_2'(S_1\Gamma) |-s s : A' -> B'}{
	\begin{matrix} W(\Gamma,s)  \\~> (S_1,A_1) \end{matrix}
	~\land~ \exists R_1.
	\left(\begin{matrix}
	...
	By definition, $\FV(\overline{S_1\Gamma}(A_1))\subseteq\FV(S_1\Gamma)$.
