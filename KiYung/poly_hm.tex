...
\citet{Damas85} (a student of Milner) published detailed theories about Milner's polymorphic
...
with mono-types) is a special case of generic instantiation. For example,
...
The first three delarative rules in Figure \ref{fig:hm}:
\rulename{Var}, \rulename{Abs}, and \rulename{App} are
...
To show this, we only need to show that ($\subseteq$) relation
holds pointwise on the type schemes in the context,
since the free variables of a context are collections
of the free variables of the type-schemes in the context.

Since $\FV(\sigma') \subseteq \FV(\sigma)$
when $\sigma' \sqsubseteq \sigma$, ... TODO .

Since the defintion of $\sqsubseteq$
...
\item[case]($t\;s$)
	...
	Recall that $\unify$ succeeds when the two types are unifiable
	and the resulting subsitutiion is a most general unifier for those
	two types. If $S_3$ were not a most general unifier, it might make
	the closures of $A_1$ and $A_2$ too specific so that $\sqsubseteq$
	relations in \ref{WcompleteAppsCase1'} and \ref{WcompleteAppsCase2'}
	no longer hold. So our choice $S_3'=S'''\circ S_3$ for
	\ref{WcompleteAppsCase2'} is a most probable candidate
	-- that is, nothing else could work if this choice doesn't work. 
	The choice $S_2' = S'''\circ S_3 \circ S_2$ for
	\ref{WcompleteAppsCase1'} is made accordingly
