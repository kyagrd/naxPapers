...
\[ \qquad \inference[GenInst]{X_1',\dots,X_m'\notin\FV(\forall X_1\dots X_n.A)}
...
the one need to observe that the free variables of a context
arise only from the type-schemes in the context. Since $\FV(\sigma') \subseteq \FV(\sigma)$
...
\item[case]($t\;s$)
        ...
        Choose $S_3'=S'''\circ S_3$, $S_2' = S'''\circ S_3 \circ S_2$.
