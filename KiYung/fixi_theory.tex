...
In this section, we show strong normalization, and logical consistency
of \Fixi.  We also give a
partial proof of the syntactic conditions necessary for well-behaved course-of-values
recursion.
...
\begin{itemize}
\item Define a notion of index-erasure that transforms \Fixi\ types to \Fixw\ types.
\item Define the meaning of a \Fixi\ term to be the meaning of its erasure.
\item Show that every well-typed \Fixi term erases to a well-typed \Fixw term.
\item Since \Fixw\ is strongly normalizing \cite{AbeMat04}, and
that index-erasure maps a valid typing judgment on a term
in \Fixi\ to a valid typing judgment on the same term in \Fixw\ implies the
...
that selects only the index variable bindings ($i^A$) from the type level context.
...
by index erasure,
for \Fixi,  is virtually the same as the strong normalization
proof for \Fi\ (in \S\ref{ssec:fi:sn}). Here, we list only the theorems
since their proofs can be trivially reconstruced by consulting the proofs
...
\emph{monotonicity witnesses} [TODO cite]. The theoretical development of
...
Conjecture \ref{conj:fixi:fmap} below. In this thesis, we only show
that this conjecture holds for simple cases (Proposition \ref{prop:fixi:fmapBaseCase}), but still encompassing a broad range of types.
...
Recall we are proving a simplified version of the conjecture we want with
...
\begin{itemize}
        ...
        When $G$ is an application of the form $X_2 G_2$, after reducting
