...
traded off against the loss in transparency that such encodings force upon
the implementation. The natural structures, which were evident
in the negative datatype, become obscured by such encodings.

A series of papers \cite{Pat93,MeiHut95,FegShe96,DesPfeSch97,bgb} has studied 
techniques that define recursion schemes directly over negative recursive types
in the conventional setting. In our recent paper \cite{AhnShe11}, we discovered
that iteration over negative recursive types can be naturally captured
as a kind-indexed family of Mendler style combinator. The \MsfIt\ combinator
(\aka\ \textit{msfcata}) at kind $*$ corresponds to the conventional recursion
combinator discovered by \citet{FegShe96} and later refined by \citet{bgb}.
...
\subsection{Formatting HOAS}\label{sec:showHOAS}
\index{Higher-Order Abstract Syntax||see{HOAS}}
\index{HOAS}
To lead up to the Mendler-style solution to formatting HOAS,
we first review some earlier work on turning HOAS expressions into strings.
...
The algorithm works, but the augmentation introduces junk.
...
\varsDef
\end{singlespace}
\caption{|msfcata0| example: String formatting function for HOAS.}
\label{fig:HOASshow}
\end{figure*}
\end{landscape}
}
...
\ExpGExamples
\end{singlespace}
Since we can build any untyped lambda expression with |Exp_g|, 
...
we passed to |z|, which is (|Var_g v|), was well behaved.
...
In Figure \ref{fig:HOASshow}, we define |Exp_g| in anticipation of
our need to write a function |showExp_g :: Exp_g -> String|,
by including a constructor |Var_g :: String -> Exp_g|.
Had we anticipated another function |f:: Exp_g -> Int|,
we would have needed another constructor |C :: Int -> Exp_g|.
...
type. How do we lift this kind of solution to Mendler style?
...
to remove the junk introduced by that augmentation (\ie, things such as
|App (Inv 1) (Inv 1)|).
...
arbitrary type |t| into values of |Exp' t| (\eg,
...
The expressions that contain |Inverse0| have more specific types
...
Using parametricity to sort out the junk introduced by the inverse
is the key idea of \citet{bgb}, and the inverse-augmented fixpoint |Mu0|
is the key idea of \citet{FegShe96}. The contribution we make in this work
is putting together these ideas in a Mender-style setting. By doing so, we are
able to define recursion combinators over types with negative occurrences,
which have well-understood termination properties enforced by parametricity.
We define four such combinators: |msfcata0|, |msfhist0|, |msfcata1|, and
|msfhist1|. The combinator |msfcata0| is the simplest. To define it,
...
the same role as |(Var_g v)| in |show'|.
...
on page \pageref{ssec:showHOASmsfcata}.  We assume that the first problem
...
doesn't allow it to be applied to constructors, as they're not abstract enough. 
Just as we couldn't define |p_m| (in Section \ref{ssec:tourNegative}), we can't
apply |z| to things such as {\small (|Lam (\ x -> x)|)}.
...
The case expression |caseSum| for the sum type is just a binary function
application. In the \Fw\ encoding, this could be omitted
...
In Figure \ref{fig:HOASshowFw}, we define both a recursive datatype
...
The situation is different from embedding of the Mendler-style iteration
into \Fw, where |Mu0|, |mcata0|, and also |In0| are embedded into \Fw\ (see
...
the embedding of natural numbers on page \pageref{pageNatFw}, \S\ref{sec:proof}).

Unfortunately, for the Mendler-style iteration with syntactic inverses,
we have not found a way to factor out |Rec0| as an \Fw-term to reuse it
for embedding data constructors of inverse-augmented recursive types.
We can only embed data constructors (|app| and |lam|) of a specific
recursive type (|Exp|). This is analogous to the situation where we can
embed any given regular recursive type in System \F, but can not factor
out |Mu| or |In| as we can do in System \Fw.

We strongly believe that there exist systematic algorithms of
...
leads to desired the embeddings would be interesting future work.
...
See Figure \ref{fig:HOASeval} (where they are not omitted)
...
\item Evaluating an HOAS application |(App f x)| lifts the function
  |f| and argument |x| to host-language values
        |(ev f)| and |(ev x)|, and uses the host-language application
...
since |Rec1| and |msfcata1| can be embedded into \Fw\ in a manner  
...
of Mendler style over conventional style in one example.
...
So, the |showHOAS| example only shows that Mendler style is
as expressive as conventional style (although it is
perhaps syntactically more pleasant than conventional style).
...
defined over indexed datatypes of arbitrary kinds. In fact, both
|msfcata1| used in |evalHOAS| and |msfcata0| used in |showHOAS|
have exactly the same syntactic definition. They differ only
...
Another example of a negative datatype is the graph with cycles and sharing %%.
...
%}
\caption{The Mendler-style open-iteration |mopenit0|,
...
embedded function and then apply that function to an ancestor that
...
In HOAS, a meta-level function (from expressions to expressions)
represents an expression with a single free variable. For example,
...
There is an open-iteration combinator at each kind (\eg, |mopenit1|
at kind |* -> *|), just like other combinators.
\citet{bgb} studied open-iterations that support
