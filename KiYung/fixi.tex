...
We discuss the embedding of primitive recursion over recursive datatypes
(\S\ref{ssec:fixi:data:pr}) and the embeddings of negative datatypes
(\S\ref{ssec:fixi:data:neg}) in \Fixi.
...
We can prove strong normalization of by erasing term-indices in the types of
\Fixi\ into \Fixw\ types. Since we know that \Fixw\ is strongly normalizing
\cite{AbeMat04}, the existence of erasure that maps a valid typing judgment on
a term in \Fixi\ to a valid typing judgment on the same term in \Fixw\ implies
