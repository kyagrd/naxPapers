...
\KYA{if there is a citation cite someone who discovered equi-recursive types
over arbitrary polarity breaks normalization
and other discussions if we had written the intro we may refer back to that
but we havn't so well let's see how it goes TODO}
...
Let ~ $\Delta =
	X_{\!F}^{0\kappa-> \kappa}, \mathbb{I}^\kappa,
	X^{0\kappa}$ ~ and ~
$\Gamma =
