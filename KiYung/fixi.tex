...
datatypes such as void, unit, pairs, sums, and existential types:\footnote{
Examples from \citet{AbeMat04}, except for the last example of $\exists_A$}
...
\label{sec:fixi:cv}
%% For positive base structures, embeddings of the iso-recursive operator
%% $\mu^{+}_\kappa$ and its and course-of-values primitive recursor $\McvPr_\kappa$
%% can be much less involved than $\mu_\kappa$ and $\MPr$. TODO
...
\In_\kappa &\;:~ \forall X_{\!F}^{+\kappa-> \kappa}.
	X_{\!F}(\mu^{+}_\kappa X_{\!F}) \karrow{\kappa} \mu^{+}_\kappa X_{\!F}\\
\In_\kappa &\triangleq
	\l t. \l s. s~\unIn_{X_{\!F}}\;\textit{id}\;\,(\McvPr\;s)\;\,t \\
\end{align*}\vskip -2.5ex
We rely on the fact that there exists
$\unIn_{F} : \mu^{+}_\kappa F \karrow{\kappa} F(\mu^{+}_\kappa F)$
for any positive base structure $F:+\kappa -> \kappa$, 
such that
\[
	(\In_\kappa \circ \unIn_F)\;r -->+ r \qquad \qquad
	(\unIn_F \circ \In_\kappa)\;t -->+ t
\]
...
\caption{Embedding of the recursive type operators ($\mu^{+}_\kappa$),
	their data constructors ($\In_\kappa$),
	and the Mendler-style course-of-values primitive recursors
	($\McvPr_\kappa$) for positive datatypes in \Fixi.}
...
\begin{array}{llcll}
N &\!\!\!= \l X^{+*}.X + \textsf{Unit} &&
\unIn_N &\!\!\!= \McvPr_{*} (\l\_.\l\textit{cast}.\l\_.
	\l x. x\;(\texttt{inL}\circ\textit{cast})\;\texttt{inR})
	\\
L &\!\!\!= \l X_a^{0*}.\l X^{+*}.(X_a\times X) + \textsf{Unit} &&
\unIn_{(L\,A)} &\!\!\!= \McvPr_{*} (\l\_.\l\textit{cast}.\l\_.
	\l x. x\;(\texttt{inL}\circ(\textit{id}\times cast))\;\texttt{inR})
	\\
P &\!\!\!= \l X_a^{0*}.\l X^{+* -> *}.
	X_a \times X(X_a \times X_a) + \textsf{Unit} &&
\unIn_{(P\,A)} &\!\!\!= \McvPr_{* -> *} (\l\_.\l\textit{cast}.\l\_.
	\l x. x\;(\texttt{inL}\circ(\textit{id}\times cast))\;\texttt{inR})
\end{array}
\]
TODO think we need notion of libniz equality and Kan extension for
	embedding base structure for term-indexed datatypes like vectors
\[
V = \l X_a^{0*}.\l i^{\texttt{Nat}}.\l X^{\texttt{Nat} -> *}.
(\exists j^\texttt{Nat}(i=\texttt{S}\,j). X_a \times X\{j\}) +
(\exists j^\texttt{Nat}(i=\texttt{Z}   ). X\{\texttt{Z}\})
\]
\[
\unIn_{(V\,A)} = TODO
\]

$\exists j^A(t).B$ is a shorthand notation for $\exists_A(\ j^A.t)B$
(see Figure TODO for the definition of $\exists_A$.

\[\text{
See Figure \ref{fig:embedMcvPr} for the embedding of the Mendler-style
course-of-values primitive recursor ($\McvPr_\kappa$)}
\]
\caption{Embeddings of unroll operations ($\unIn_F$)
