...
avariant ($\mathsf{0}$).\footnote{``invariant'' is more often used
	(also in \cite{AbeMat04}, but we think it is a quite misleading jargon,
	since $\mathsf{0}$ means that it \emph{does not care} about polarities
	rather than maintaining some unchanging properties about polarities.}
TODO

\paragraph{Equi-recursive type operator $\fix$}
TODO
...
~\\
\MPr_\kappa &\;:~
	\forall X_{\!F}^{\mathsf{0}\kappa-> \kappa}.\forall X^\kappa.
	(\forall {X_r}^{\!\!\kappa}.
...
\text{The type of $r$ can be expanded by the defintion of $\fix$
	and the equi-recursive equality rule on $\fix$ as folllows:}\]
	\vskip-6ex
\begin{multline*}\,
...
\inference{
	\inference{
	X_{\!F}^{\mathsf{0}\kappa-> \kappa}, X^{\mathsf{0}\kappa},
	\mathbb{I}^\kappa
	; \;
	s: (\forall {X_r}^{\!\!\kappa}.
	 (X_r \karrow{\kappa} \mu_\kappa X_{\!F}) ->
...
Let ~ $\Delta =
	X_{\!F}^{\mathsf{0}\kappa-> \kappa}, \mathbb{I}^\kappa,
	X^{\mathsf{0}\kappa}$ ~ and ~
$\Gamma =
