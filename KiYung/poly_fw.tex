...
We already know how to encode the base structure: in the encoding above,
we can eliminate to an arbitrary result type $X$. For $\mu$, we introduce
yet another variable $X_a$ so that we can eliminate to an arbitrary result
type $X_a$. So, it would be of the following form:
\[ \forall X_a^{*}.(\;\dots\;\dots\;\dots\;\dots\;\dots (X_r + \textit{Unit}) -> X_a) -> X_a \]
...
the base structure $((X_r + \textit{Unit}) -> X_a)$.
...
\[ \forall X_a^{*}.
	(\forall X_r^{*}.(X_r -> X_a) -> (X_r + \textit{Unit}) -> X_a) -> X_a \]
...
\mu =
 ...
 ~:\; (* -> *) -> *
\]
Note that $X_r$ is also universally quantified. See Chapter \ref{ch:mendler}
