...
[| \forall X^\kappa . B |]_\xi
	&= \bigcap_{\mathcal{F}\in\SAT_\kappa} [|B|]_{\xi[X\mapsto\mathcal{F}]}
		\qquad\qquad\qquad (X\notin\dom(\xi)) \\
[| \l X^\kappa . F |]_\xi
	&= \bbl(\mathcal{G} \in \SAT_\kappa) . [|F|]_{\xi[X\mapsto\mathcal{G}]}
		\qquad\quad~ (X\notin\dom(\xi)) \\
...
\item[Interpretation of kinding and typing contexts]
	...
	($\xi$ and $\rho$):
\begin{align*}
[| \Delta        |] &= \{ \xi \in \dom(\Delta) -> \bigcup_{\kappa} \SAT_\kappa \mid \xi(x)\in\SAT_{\Delta(x)} ~\text{for all}~x\in\dom(\Delta) \} \\
...
System \F. However, we need to generalize the interpertation of types to
the interpretation of type constuctors. In the strong normalization proof
...
\end{align*}
The interpretation of type constructors, contexts, and terms of System \Fw\ 
are illustrated in Figure \ref{fig:interpFw}. 

We use the Curry-Church-style System \Fw\ to present
the strong normalizing proof.
