...
A recent popular extension to the GHC Haskell compiler is GADTs \cite{She05}.
In our nested examples, the variation of type indices always occurred in
the arguments of the data constructors. GADTs are indexed datatypes,
where the index may vary in the result types of the data constructors.
Haskell's normal |data| declaration, which uses an ``equation'' syntax, 
makes the assumption that the result types of every constructor
are the ``same'' type with no variation. GHC's GADT datatype extension
 is more expressive than the usual |data| declaration in equational form.
The GHC compiler extends the |datatype| syntax, so that each datatype
constructor is given its full type annotation. The datatype definition
for vectors (or size-indexed lists) is a prime example:
\index{vector}
...
inhabited by any value. They are only intended to be used as indices.
...
\end{code}
Note, the type argument varies in the second argument of |CB|, which is
|Bush (Bush i)|, but both the result types of |NP| and |CP| are |Bush i|.

In Figure \ref{fig:vec}, 
we define the vector datatype |Vec| as |Mu1 (V p) i|, in Mendler style.
...
using the Mendler-style iteration combinator |mcata1| at kind $* -> *$.
We can express the |switch2| function that switches every two elements of
the given vector, in Mendler style, using the course-of-values iteration
combinator |mhist1| at kind $* -> *$.
\index{Mendler-style!course-of-values iteration}
The definitions of |mcata1| and |mhist1| are exactly the same as
the definitions of |mcata0| and |mhist0|, except that |mcata1| and |mhist1|
...
than defining functions for regular datatypes using |mcata0| and |mhist0|.
