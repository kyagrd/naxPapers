...
\end{align*}
\[ \textbf{typing rules}
	\qquad\qquad\qquad\qquad \framebox{$\Gamma |- t : A$} \]
\vspace*{-1em}
\begin{align*}\quad
...
\[ \textbf{reduction rules}
	\qquad\qquad\qquad\quad \framebox{$t --> t'$} \]
\vspace*{-1em}
\begin{align*}\quad
...
& \inference[\sc App]{\Gamma |- t : A -> B & \Gamma |- s : A}{\Gamma |- t~s : B} \\
\end{align*}
\[ \textbf{reduction rules} \] \vspace*{-1em}
\begin{align*}\quad
...
\begin{figure}
\begin{singlespace}
\[ \textbf{typing contexts} \qquad \Gamma ~::=~ \cdot ~\mid~ \Gamma, x:A \]
\[ \textbf{well-formed typing contexts}
	\quad \framebox{$ |- \Gamma $}\]
\[ \qquad \inference[\sc TyCtxEmp]{}{ |- \cdot }
   \qquad \inference[\sc TyCtxExt]{ |- \Gamma & \Gamma |- A }
                                  { |- \Gamma, x:A }
			  (x\notin \mathop{\mathrm{dom}}(\Gamma)) \]
\caption{Syntax and well-formedness rules for typing contexts}
\label{fig:tyctx}
\end{singlespace}
\end{figure}
...
Some interesting properties of Church style, which does not hold
...
from the well-formedness of the typing context (see Figure \ref{fig:tyctx}).
...
under a well-typed context is still unique. That is,
