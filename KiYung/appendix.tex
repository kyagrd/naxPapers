...
basic knowlege of logic (\eg, concepts of axioms, inference, and proofs)
...
and the Agda proof \textit{id}, which proves that $A$ is true
...
We illustrate a intuitionistic natural deduction
(and its corresponding simply typed lambda calculus)
with implication $->$ (function type) and falsity $\bot$ (void type) below.
\[
\begin{array}{ccc}
	\text{Intuitionistic natrual deduction} & &
	\text{Typing rules of a simply typed lambda calculus} \\ \\
\inference{\bot \in \Gamma}{\Gamma |- A} & (\bot_\textsc{E})  &
\inference{x:\bot \in \Gamma}{\Gamma |- A} \\ \\
...
Prawitz \cite{Pra65} normal deduction
