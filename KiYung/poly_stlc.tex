...
type binding ($x:A$) in a given context ($\Gamma$), in both Church style
and Curry style. Although typing is not unique in the Curry-style STLC,
the limitation is still the same since the \rulename{Var} rule is basically
the same as in Church style. It is possible to give many typings for terms
...
many different types, in fact, arbitrary type you need, in each occurrences
of its use. We will introduce several well-known polymorphic lambda calculi
