...
\paragraph{Properties of STLC (that hold in both styles):}~\\
...
over reduction) by induction on the derivation of the reduction rules.
...
can take an evaluation step. An evaluation is a certain subset of
the reduction relation, which is often deterministic. Values are
terms that meet certain syntactic criteria, which is intended not
to take further evaluation steps. In such a setting, type safety
means subject reduction together with progress. However, as a calculus
considering reduction and normal terms, rather than evaluation and values,
type safety is no more than subject reduction.

Strong normalization is another important property of STLC.
The proof strategy we present here is to define a strongly normalizing set of
terms, which may or may not be well-typed, and show that all well-typed terms
belong to that set.
\KYA{we do this for Curry style but can we can do it similarly for Church style
TODO cite Geuvers ``a short and flexible proof of String Normalization for CC''
and also Matthes' CSL 04 paper on \Fixw}
...
\inference{t[s/x]~s_1~\cdots~s_n\in \mathcal{A} & s\in\mathcal{A}}
	  {(\lambda x.t)~s~s_1~\cdots~s_n \in \mathcal{A}}
\]
...
We define an arrow operator on saturated sets
...
\begin{align*}
[| \iota  |] &= \text{the minimal saturated set} \\
[| A -> B |] &= [| A |] -> [| B |]
\end{align*}
...
\begin{proof}
We can prove above by induction on the typing derivation ($\Gamma|- t:A$).
...
\]
By induction, we know the following:
