...
the Mendler style that look alot like the programs written using general recursion
...
We identify several open problems. We introduce two of them here. We
discuss both of them in greater detail later in the thesis.

\paragraph{Handling different interpretations of $\mu$ in one language system:}
Nax provides several mechanisms to describe recursive data, and for
each mechanism an induction principle (or recursion scheme)
used to describe recursive computations over such data. These schemes are all
motivated by concrete examples, which explains the need for multiple schemes.
However, there is theoretical difficulty describing multiple interpretations of
$\mu$ in one system.
...
There are different encodings of $\mu$, one for each recursion scheme.
Some recursion schemes subsume others
(\ie, the more expressive one can simulate the other).
We know of no Mendler-style recursion scheme that subsumes
all the recursion schemes supported in Nax.
...
(\ie, the  existence of a map) for a type constructor from just its polarized kind,
...
with the manual derivation for every datatype on which they might want to perform
course-of-values recursion. If the type system can automatically
categorize datatypes that have maps from their polarized kinds this burden
can be alleviated.
