...
\paragraph{}
From a bird's-eye view, the following chapters back up these claims up as follows:
...
Term-indexed types support (2), because they can statically track program properties. For instance
the size of data structures can be tracked by using a natural number term in their types.
...
Mendler-style recursion schemes also also support (4)
because their termination is type-based -- no need for an auxiliary termination
checker.
...
In next section, we summarize important ideas mentioned in our thesis
discussed above.
...
We will develop each of these ideas in full detail in the following chapters.
(see \S\ref{sec:sec:intro:overview} for the overview of chapter organization).
...
some of which we have discovered. Throughout this dissertation,
we show that the Mendler-style recursion combinators are
...
We identified some open problems. We introduce two of them here:
handling different interpretations of $\mu$ and deriving positivity
(or monotonicity) from polarized kinds.

\paragraph{Handling different interpretations of $\mu$ in one language system:}
Nax provides several different induction principles, or recursion schemes,
that can express useful patterns of computations. This gives convenience to
programmers since these patterns of computations were motivated by examples. 
However, there is a theoretical difficulty of handling different semantics of
$\mu$ in one system.

Recall that we embed datatypes as functional encodings into
our indexed type theory. Recursive datatypes and recursion schemes of Nax
are embedded into indexed type theory using Mendler-style encoding.
In Mendler style, one encodes the recursive type operator $\mu$
and its eliminator, which is is one of the Mendler-style recursion schemes.
So there are different encodings of $\mu$, one for each recursion scheme.
Some recursion scheme subsumes another
(\ie, more expressive so that it can simulate another), but
there exists no known Mendler-style recursion scheme that can subsume
all the other recursion schemes supported in Nax.
For instance, iteration (\MIt) can be subsumed by either 
iteration with a syntactic inverse (\MsfIt) or primitive recursion (\MPr),
but there are no known recursion scheme that can subsume both \MsfIt\ and \MPr.

However, we strongly believe that it is okay to use apply \MsfIt\ to
the result of \MPr\ (when \MPr produces a recursive value) and vice versa,
since the different interpretations of $\mu$ only matters during
the computation of recursion schemes. That is, one may consider
that (recursive) values resulting from different recursion schemes
sahre the same meaning of $\mu$. The theoretical justification for
this is still ongoing work.

\paragraph{Deriving positivity (or monotonicity) from polarized kinds:}
One can extend the kind syntax of arrow kinds in \Fw\ with polarity
($p\kappa_1 -> \kappa_2$ where the polarity $p$ is either $+$, $-$, or $0$)
to keep track of whether a type constructor argument is used in
covariant (positive), contra-variant (negative), or
mixed-variant (both positive and negative) positions.
It is yet an open problem whether it is possible to derive monotonicity
(\ie, existence of a map) of a type constructor just from the polarized kind,
without examining the type constructor definition.

We identified a useful application of this open problem in the context
of Mendler style encoding.
We discovered an embedding of Mendler-style course-of-values recursion in
a polarized system for positive (or monotone) type constructors that have maps.
That is, once you can show the existence of a map for a datatype,
it is guaranteed that course-of-values recursion terminates for that datatype.
However, in a practical language system, it is not desirable to burden users
to manually derive a map for every datatype they want to perform
course-of-values recursion. If the type system can automatically
