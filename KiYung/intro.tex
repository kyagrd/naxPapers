...
This dissertation makes contributions in four related areas.
\begin{itemize}
\item[1.]
...
\item[2.] It establishes a meta-theory for  \emph{term-indexed types}
        (Part~\ref{part:Calculi}),

\item[3.] It designs a practical language (with an implementation) \emph{in the sweet spot}
        between programming and logical reasoning (Part~\ref{part:Nax}), and

\item[4.] It identifies several interesting open problems. TODO.
\end{itemize}

\subsection{Contributions related to the Mendler-style}
We organize a hierarchy of Mendler-style recursion schemes
in two dimensions. The first dimension is the abstract operations they support. For instance,
...
For example \texttt{Nat} has kind \texttt{*}, while \texttt{List}
has kind \texttt{* -> *}. Each recursion scheme is actually a
...
for negative datatypes.  Second, we study the termination behavior
of Mendler-style recursion schemes. Some recursion
schemes (\eg, \MIt, \MsfIt) always terminate for any recursive type, while
others (\eg, \McvPr) only terminate for certain classes of recursive types.
Third, we extended all Mendler-style recursion schemes to be
expressive over term-indexed types. The
Mendler style has been studied in the context of \Fw\ (and several extensions)
which can express type-indexed types. To extend Mendler-style recursion schemes
to be expressive over term-indexed types, we report on several theories for
calculi (\Fi\ and \Fixi)  supporting term-indexing. This is an
...
Mendler-style course-of-values recursion (\McvPr) is reported in the literature,
but the calculus that can embed \McvPr\ was unknown.
We embed Mendler-style course-of-values recursion into
the lambda calculus \Fixi.
...
%% When restricted to its use over positive (or, monotone) datatypes only,
...
(type inference, arbitrary datatypes).
