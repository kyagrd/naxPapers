...
supporting \emph{term indices} in types, which sits between
polymorphic types and dependent types.

In the following section, we clarify our design goals of Nax,
that is, what we mean by the sweet spot between programming languages
and reasoning systems, and claim our thesis that our design choices
for Nax are reasonable to achieve those goals.

\section{Thesis}\label{sec:intro:thesis}
We characterise four key features desirable for a system in the sweet spot
that unifies programming and reasoning, as follows:
\begin{enumerate}[(1)]
 \item \textbf{Convenient programming}
	 by supporting major constructs of
	 modern functional programming languages, such as
         parametric polymorphism, recursive datatypes,
	 recursive functions, and type inference,
 \item \textbf{Expressive logic}
	 that can specify fine-grained program properties as types and
         witness proofs of those properties by writing a program
         (the Curry--Howard correspondence),
 \item \textbf{Small theory} based on a minimal foundational calculus that is
	 expressive enough to embed all the constructs above
	 (for both programming and logic) and is also logically consistent
...
\paragraph{}
In a bird's-eye view, our thesis is backed up as follows.
Mendler-style recursion schemes support (1) since they are based on
parametric polymorphism and well-defined over wide range of datatypes.
Term-indexed types support (2), for instance, one can statically track
the size of data structures by using a natural number terms in their types.
To support (3), we design appropriate foundational calculi, which extends
from polymorphic lambda calculi with term-indexed types while maintaining
the desired properties. Mendler-style recursion schemes also also support (4)
since their termination is type-based -- no need for an auxiliary termination
checker.
...
In the next section, we give summaries of the preliminary concepts,
which in the thesis description such as Curry--Howard corresponds,
...
TODO say that this is a short summary and point to later chapters
related to each of the subsections
...
is that moderate extensions to the polymorphic calculi is a better candidate
than dependently typed calculi for the basis of a practical programming system.
Recall that we aim for a unified system for programming as well as reasoning.
Language design based on indexed types can benefit from
existing compiler technology and type inference algorithms for
functional programming languages. In addition, theories for datatypes become
much more simple than full-fledged dependent types, since datatypes can be
encoded as functions in moderate extensions of polymorphic lambda calculi.

Functional programming language implementation technology based on
polymorphic calculi have already grown mature. There exist industrial
strength implementations such as the Glasgow Haskell Compiler (GHC),
whose intermediate core language is an extension of \Fw.
Our term-indexed calculi in Part \ref{part:Calculi} are closely
related to \Fw\ by the index erasure property.

Type inference algorithms in functional programming languages are usually
based on certain restrictions on Curry-style polymorphic lambda calculi
that are designed to avoid higher-order unification during type inference.
We are able develop a conservative extension of Hindley--Milner type inference
for Nax (Chapter \ref{ch:naxTyInfer}), since it is based on our calculi, which
extends higher-order polymorphic lambda calculi with erasable term indices
(Part \ref{part:Calculi}). On the other hand, dependently typed languages are
usually based on bidirectional type checking, which requires annotations on
top level definitions, rather than Hindley--Milner-style type inference.

In dependent type theories, datatypes are not self-contained in
the pure lambda calculi. That is, datatypes must be supported as
primitive constructs, rather than functional encodings, in order to
enjoy full ramifications of dependent datatypes. When one tries to
give functional encodings (\eg, Church encodings) for datatypes, one
soon realizes that the induction principles (or, dependent eliminators)
for those datatypes representations cannot be derived within
the pure dependent calculi [TODO cite H. Geuvers 2001].
...
On the contrary, in polymorphic type theories, all imaginable datatypes
within the calculi can have functional encodings (\eg, Church encodings).
