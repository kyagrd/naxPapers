...
We explain some of the preliminary concepts necessary to understand
the Curry--Howard correspondence in the following subsections.
...
We also assume that the readers understand basic concepts of logic
(\eg, axioms, inference, and proofs) but not necessary familiar with
...
and connectives, for constructing proofs.
...
$\beta$-reduction, which describes the computation over function applications,
...
lambda calculus. For instance, we can define the interpretation $[|A|]$ for
the proposition $A$ as the set of terms of type $A$ in the lambda calculus.
In a model following such interpretations, empty set would be the falsity value.
...
Since the scope of our research is for reasoning systems, which are based on
...
counterparts.
%% Because of this correspondence, we will use \emph{lambda calculus}
...
In type theory, we can understand the datatype above as a sum of products
...
the type operators, sum ($+$) and product ($\times$), and
...
We summarized the different characteristics between the datatypes
in functional languages and the datatypes in reasoning systems
in Figure\;\ref{fig:datadiff}. Our approach on datatypes is to find
...
There are two ways of ensuring normalization for datatypes and
...
approach, is taken by most reasoning systems (\eg, Coq, Agda) and the studies
...
approach, puts no restrictions on the definition of datatypes, but instead,
carefully restricts the use of data values. In particular, the decomposition
(\ie, elimination, or, pattern matching) of recursive values are restricted.
...
the Curry--Howard correspondence for datatypes, because datatypes are encoded
using the basic primitives of the lambda calculus, for which we know,
the Curry--Howard correspondence already hold. In addition, there is no need
for any extra mechanism, other than type checking, to ensure termination of
the recursion schemes. We take this non-conventional approach for our language
system design.
...
%% unrestricted general recursion (at the term-level) is not supported. 
...
use of a recursive type operator, $\mu$, at the type-level. 
...
The formation rule {\small($\mu$-form)} remains the same as for
...
{\small($\mu$-elim)}. That is, we are not allowed to pattern match over
...
the values of recursive types through the Mendler-style recursion combinators.
...
recursive types (a more conventional approach). The impact of
this design choice is that it enables our language system to include
...
than the programs written in conventional style.
...
definitions, unlike Hindley--Milner type inference.
...
a dependently typed Mendler-style primitive recursion, which is well-defined
...
Since \Fi\ and \Fixi\ are strongly normalizing and logically  consistent,
...
simple since we do not need extra termination checking algorithms.
...
which leads to a single interpretation of $\mu$. Unfortunately,
we know of no Mendler-style recursion scheme that subsumes
all the other recursion schemes in Nax. For instance, iteration (\MIt)
is subsumed by primitive recursion (\MPr), but \MPr\ does not subsume
iteration with a syntactic inverse (\MsfIt) or the other way around.
...
%% is possible is still ongoing work.
...
\paragraph{Part \ref{part:Mendler} (Mendler style)}\hspace{-1em}
\index{Mendler-style}
introduces
...
develops theories for term-indexed types. We formalize two term-indexed
lambda calculi, which extends their underlying polymorphic calculi that
support type indices only.
...
these term-indexed calculi using their index erasure properties.
