...
We have crafted these programs to look as similar to one another as possible
by choosing the same identifiers and syntax structure whenever
possible. So, anyone already familiar with Haskell-like languages
or Agda-like languages could easily understand our Nax programs
just by comparing them with the programs on the left or on the right.
The features unique to Nax, which are used in this chapter,
are summarized in Table\;\ref{tbl:naxfeatures} (review
Chapter \ref{ch:naxFeatures} for further details).
...
\end{itemize}
We adopt the examples from Conor McBride's keynote talk \cite{McBride12}
...
\label{ssec:eval}
\index{evaluator!type-preserving}
...
Once the evaluator is type checked, we are confident that the evaluator is
...
(|BV b|).  Note that the both the |Expr| and the |Val| datatypes are indexed by
...
\index{term index}
Curly braces in the Nax code above indicate the use of term indices in types.
...
We will instantiate |Path| into three different types of lists:
plain lists, length-indexed lists 
...
is a binary relation describing legal transitions (\ie, |x {i} {j}|
is inhabited if one can legally step from |i| to |j|).
The arguments |i : iota| and |j : iota| represent the initial and
final vertices of |Path|. A term of type |Path x {i} {j}| witnesses
...
First, |pNil : Path x {i} {i}| witnesses an empty path
(or $\epsilon$-transition) from a vertex to itself, which always exists
...
and which witnesses a step with no interesting information.
...
witnessing steps of size one. The relation (|ElemV a|) counts down
exactly one step, from |succ n| to |n|, as described in the type signature of
|MkElemV : a -> Elem a {`succ n} {n}|. Then, we define |Vec a {n}| as
a synonym |Path (ElemV a) {n} {`zero}|, counting down from |n| to |zero|.
In Nax, in a declaration, backquoted identifiers appearing inside index terms
enclosed by braces refer to functions or constants in the current scope
(\eg, |`zero| appearing in |Path (ElemV a) {n} {`zero}| refers to
the predefined |zero : Nat|). Names without backquotes (\eg, |n| and |a|)
...
In Agda, there is no need to define wrapper datatypes such as |Elem| and |ElemV|
...
a list of type representations (|List Ty|).\footnote{
	...
	Unfortunately, this is not the case in Haskell.
	...
	the two approaches of Nax and Haskell differ
