...
into strongly normalizing $\lambda$-calculi. The stratgey is to develop
a sequence of calculi of increasing expressiveness. Several papers by other
researchers have
begun this process. Mendler's orginal work extended System \textsf{F},
Abel and Mathhes extended System \Fw\ to get \Fixw. In my thesis I will
follow in these footsteps by introducing System \Fi\ (a more expressive
extension to \Fw) and System \Fi\ (an extension to \Fixi).
...
by allowing predefined types ($\tau : *$) to appear in the domain of arrow kinds
($\tau -> *$), as follows:
\begin{align*}
...
the three key properties of \Fi. I will provide proofs in my thesis of these properties.
\begin{description}
...
Interesting propoerties of \Fixw\ include the ability to define constant
time predecessors.
...
addition to proving the three key properties of Systems \Fi\ and Sytem
...
The embedding of a kind-indexed family of Mendler-style operators
is a pair of translations -- a translation of the recursive type operator ($\mu^\kappa$),
and a translation of the Mendler style recursion combinator. 
...
The details of the embedding may be different for each family even though some of
...
of \MIt\ and \MsfIt\ are different. In general,
the translation of the recursive type operator $\mu^\kappa$ is different for
each family, even though the target language of the translation may be the same.
