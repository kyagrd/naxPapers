...
One of the reasons the search for interesting examples has been neglected
is that techniques exist for encoding negative recursive types into positive recursive ones.
Recall that the conventional iteration and
primitive recursion normalize for positive datatypes. Thus, in such
encodings, we can use standard techniques. What we gain by using such encodings must be 
traded against the loss in transparency that such encodings force upon the implementation.
The natural properties, which were evident in the negative datatype, become
obscured by the encoding. 

\paragraph{A new family of Mendler style recursion combinators for negative datatypes.}
Identifying additional Mendler style operators that work
naturally, and are more expressive than iteration and primitive recursion, has been
an important result of my thesis work. While it is known
that iteration and primitive recursion terminate for all datatypes
\cite{AbeMatUus05,AbeMat04}, they are not particularly expressive over negative datatypes.

A series of papers
\cite{Pat93,MeiHut95,FegShe96,DesPfeSch97,bgb} has studied 
...
as a kind-indexed family of Mendler style combinators. The \verb+msfcata+
...
In the special case that a datatype uses only positive occurrences (the normal case
for first order data) more terminating Mendler style operators are possible.
We have identified new ones, adapted old ones, and organized them all into a natural
hierarchy that explains how all are instances of a common pattern.
However, we know of no guarantee that every principled
recursion scheme has a Mendler style operator, so there is much work still to be done.



\paragraph{Examples involving indexed datatypes.}
We \cite{AhnShe11} have illustrated that Mendler-style
recursion combinators can be defined for indexed datatypes, beyond the scope of
nested datatypes. Previous work 
that promoted the expressiveness and generality of the Mendler style 
\cite{AbeMatUus03,AbeMatUus05,AbeMat04}
used examples involving nested datatypes, a particularly simple kind of 
indexing. To handle nested datatypes
the translation of the Mendler-style into System F, had to
be extended to \Fw. 
Our work \cite{AhnShe11} includes examples involving Generalized Algebraic
DataTypes (GADTs), and examples mutual recursion (between types) encoded in terms of GADTs.
However, we only gave an informal argument of their normalization.
To translate Mendler-style combinators over GADTs into a know normalizing calculus
we have to extend \Fw, and prove that this new calculus is also
strongly normalizing. I have identified what the extensions must look like
and have been developing the theoretical background necessary to accomplish this task.
...
express what GADTs express in a natural way. The type indices
used in GADTs are often really intended to be values at term level,
but are just simulations of such terms at type level. Within System \Fi, we can rigorously
...
You can find a summary of \Fi\ in this mid-course document in \S\ref{sec:Nax}.
